...
resource "null_resource" "getdockercompose" {
  ...
  provisioner "local-exec" {
     command = "ls /root/.ssh/ && sudo scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${digitalocean_droplet.terraformnetmakerserver.ipv4_address}:/root/docker-compose.yml ."
  }
}
